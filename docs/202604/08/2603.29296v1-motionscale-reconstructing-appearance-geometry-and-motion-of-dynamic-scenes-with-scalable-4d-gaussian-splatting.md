...
tags: ["query:3dgen"]
score: 8.0
evidence: 从单目视频重建动态场景的几何与运动
tldr: 针对单目视频中复杂动态场景的 4D 重建难题，本文提出 MotionScale 框架。该方法基于 4D 高斯泼溅技术，通过聚类中心基变换构建可扩展运动场，以捕捉多样化的运动模式。结合背景扩展与前景传播的渐进式优化策略，有效解决了长序列重建中的几何畸变与运动不一致问题，在真实世界基准测试中实现了高保真的结构与运动恢复。
source: arxiv
...
motivation: 现有神经渲染方法在处理复杂动态场景的单目视频时，难以兼顾准确的 3D 几何恢复与长期的运动一致性。
method: 采用聚类中心基变换参数化运动场，并引入包含背景扩展（处理新区域与阴影）和前景传播（三阶段细化运动）的渐进式优化策略。
result: 实验证明，MotionScale 在重建质量和时间稳定性上显著超越了当前最先进的方法，能够处理大规模场景和长序列视频。
conclusion: 该研究通过可扩展的运动建模与解耦的优化流程，为实现高质量、高稳定性的动态 4D 场景重建提供了新途径。
---

## 摘要
从单目视频中真实地重建动态 4D 场景对于理解物理世界至关重要。尽管神经渲染近期取得了进展，但现有方法在复杂环境中往往难以恢复准确的 3D 几何和时间一致的运动。为了应对这些挑战，我们提出了 MotionScale，这是一个 4D 高斯泼溅（Gaussian Splatting）框架，能够高效扩展到大规模场景和长序列，同时保持高保真的结构和运动连贯性。我们方法的核心是一个可扩展的运动场，由以聚类为中心的基变换（cluster-centric basis transformations）参数化，这些变换能够自适应地扩展以捕捉多样且演变的运动模式。为了确保在长时间跨度内的鲁棒重建，我们引入了一种渐进式优化策略，包含两个解耦的传播阶段：1）背景扩展阶段，适应新出现的可见区域，优化相机位姿，并显式建模瞬态阴影；2）前景传播阶段，通过专门的三阶段细化过程强制执行运动一致性。在具有挑战性的真实世界基准测试上的广泛实验表明，MotionScale 在重建质量和时间稳定性方面均显著优于现有最先进的方法。项目页面：https://hrzhou2.github.io/motion-scale-web/。
