...
tldr: 针对辐射场重建模型（如3DGS）在低功耗设备上难以高效传输和渲染的问题，本文提出了DiffSoup。该方法将辐射场表示为由少量具有神经纹理和二值透明度的三角形组成的“汤”（无结构集合）。通过随机透明度掩码实现直接可微的光栅化，无需平滑处理即可稳定训练。DiffSoup支持标准深度测试，能无缝集成到传统图形管线中，在移动端实现极高压缩率下的交互式渲染。
selection_source: fresh_fetch
motivation: 现有的辐射场表示参数量巨大，难以在资源受限的平台和网络环境下进行高效传输与实时渲染。
method: 提出一种由少量三角形组成的无结构集合表示，结合神经纹理和二值透明度，并利用随机透明度掩码实现直接可微的光栅化。
result: 该方法实现了极高程度的模型简化，并能通过标准图形管线在笔记本和移动设备上流畅运行。
conclusion: DiffSoup通过创新的可微光栅化方案，为辐射场在通用图形硬件上的极端简化与高效部署提供了有效途径。
---
