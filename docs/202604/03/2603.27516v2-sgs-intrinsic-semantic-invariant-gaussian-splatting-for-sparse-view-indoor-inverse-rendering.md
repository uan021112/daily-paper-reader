...
tldr: 针对室内场景在稀疏视角下逆渲染质量差的问题，本文提出SGS-Intrinsic框架。该方法利用语义和几何先验构建稠密且一致的高斯语义场，为逆渲染奠定基础。通过结合混合光照模型、材质先验及去阴影模型，实现了材质与光照的精确解耦，并引入光照不变性约束增强鲁棒性。实验证明，该方法在重建精度和逆渲染质量上均优于现有基于3DGS的方法。
selection_source: fresh_fetch
motivation: 现有的基于3D高斯泼溅的逆渲染方法在稀疏视角的室内环境下难以实现高质量的几何重建和材质解耦。
method: 利用语义和几何先验构建一致的高斯语义场，并结合混合光照模型与去阴影模型实现光照不变的材质恢复。
result: 在基准数据集上的实验表明，该方法在重建保真度和逆渲染质量方面均显著优于现有的3DGS逆渲染技术。
conclusion: SGS-Intrinsic通过引入语义一致性和光照不变性约束，有效解决了稀疏视角下室内场景的逆渲染难题。
---
