...
tags: ["query:ff3dr"]
score: 10.0
evidence: 通过单次前向传播重建3D HDR场景的前馈网络
...
method: 设计了用于多曝光融合的几何引导外观建模和用于场景特定色调映射的元网络，并构建了包含168个场景的HDR-Pretrain数据集。
result: InstantHDR在单次前馈模式下实现了比优化方法快约700倍的重建速度，且合成质量与当前最先进的方法相当。
conclusion: 该研究通过前馈框架显著提升了HDR 3D重建的效率，为处理未校准的多曝光图像集合提供了高效且通用的解决方案。
---
