...
tldr: 针对脑肿瘤MRI分割中病灶空间稀疏且现有方法忽视临床先验的问题，本文提出PGR-Net。该网络通过数据驱动的空域先验集捕捉病灶分布，引入分层Top-K ROI决策机制提升定位精度，并结合WinGS-ROI模块强化特征学习。在多个主流数据集上，PGR-Net以仅8.64M的参数量实现了领先的分割性能，为临床诊断提供了高效可靠的工具。
selection_source: fresh_fetch
motivation: 脑肿瘤在MRI图像中占比极小导致严重的定位稀疏性，且现有模型往往忽略了临床观测到的肿瘤发生空间先验，导致背景计算冗余。
method: 提出一种显式ROI感知框架，利用数据驱动先验引导分层Top-K ROI决策，并结合WinGS-ROI模块生成中心增强的引导图以优化特征学习。
result: "在BraTS和MSD等多个数据集上，PGR-Net仅需8.64M参数即可在全肿瘤区域取得高达91.82%的Dice分数，优于现有主流方法。"
conclusion: PGR-Net通过整合空间先验与ROI推理机制，有效解决了脑肿瘤分割中的稀疏性挑战，实现了高精度与轻量化的平衡。
---

## 摘要
脑肿瘤 MRI 分割对于临床诊断和治疗规划至关重要，能够实现准确的病变检测和放射治疗目标勾画。然而，肿瘤病变仅占据体积空间的一小部分，导致严重的空间稀疏性，而现有的分割网络往往忽略了临床观察到的肿瘤发生空间先验，导致在广泛的背景区域上产生冗余的特征计算。为了解决这一问题，我们提出了 PGR-Net（先验引导 ROI 推理网络）——一种显式的 ROI 感知框架，它结合了数据驱动的空间先验集，以捕获肿瘤病变的分布和尺度特征，为更稳定的分割提供全局引导。利用这些先验，PGR-Net 引入了一种分层 Top-K ROI 决策机制，在编码器层中逐步选择置信度最高的病变候选区域，以提高定位精度。我们进一步开发了 WinGS-ROI（窗口化高斯空间衰减 ROI）模块，该模块使用带有空间衰减函数的多窗口高斯模板来生成中心增强的引导图，从而引导整个网络的特征学习。利用这些 ROI 特征，采用窗口化 RetNet 骨干网络来增强定位的可靠性。在 BraTS-2019/2023 和 MSD Task01 上的实验表明，PGR-Net 在仅使用 8.64M 参数的情况下，性能始终优于现有方法，在全肿瘤（Whole Tumor）区域分别达到了 89.02%、91.82% 和 89.67% 的 Dice 分数。代码可在 https://github.com/CNU-MedAI-Lab/PGR-Net 获取。
