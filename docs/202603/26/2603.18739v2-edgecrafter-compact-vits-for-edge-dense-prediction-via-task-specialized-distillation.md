...
tags: ["query:partseg"]
score: 7.0
evidence: 用于边缘端实例分割的紧凑型 ViT 框架
tldr: 针对边缘设备上紧凑型ViT在密集预测任务中表现不如CNN的问题，本文提出EdgeCrafter框架。该框架通过任务专用蒸馏和边缘友好型编解码器设计，构建了ECDet、ECInsSeg和ECPose等模型。实验证明，在目标检测、实例分割和姿态估计任务中，EdgeCrafter在保持极低参数量的情况下，性能显著优于YOLO系列等主流模型，证明了紧凑型ViT在边缘端的竞争力。
selection_source: fresh_fetch
motivation: 紧凑型ViT在边缘密集预测任务中因缺乏任务特定表征学习，导致其性能与效率的平衡难以超越传统的CNN架构。
method: 提出EdgeCrafter框架，采用任务专用蒸馏技术优化紧凑骨干网络，并结合边缘感知的编解码器设计。
result: ECDet-S在COCO上以不足10M参数达到51.7 AP，ECPose-X在姿态估计上显著超越了经过大规模预训练的YOLOv8-Pose-X。
conclusion: 通过任务专用蒸馏和边缘优化设计，紧凑型ViT能够成为边缘密集预测任务中极具竞争力的实用方案。
---

## 摘要
由于计算和内存的严格限制，在资源受限的边缘设备上部署高性能密集预测模型仍然具有挑战性。在实践中，用于目标检测、实例分割和姿态估计的轻量级系统仍由基于 CNN 的架构（如 YOLO）主导，而紧凑型视觉 Transformer (ViTs) 即使在大规模预训练下，也往往难以实现同样强大的精度-效率权衡。我们认为，这种差距很大程度上归因于小规模 ViT 中任务特定表示学习的不足，而非 ViT 与边缘密集预测之间存在固有的不匹配。为解决这一问题，我们推出了 EdgeCrafter，这是一个用于边缘密集预测的统一紧凑型 ViT 框架，其核心是 ECDet——一种由蒸馏紧凑骨干网络和边缘友好型编码器-解码器设计构建的检测模型。在 COCO 数据集上，ECDet-S 仅使用 COCO 标注，在参数量少于 10M 的情况下达到了 51.7 AP。在实例分割方面，ECInsSeg 在使用显著更少参数的情况下，实现了与 RF-DETR 相当的性能。在姿态估计方面，ECPose-X 达到了 74.8 AP，显著优于 YOLO26Pose-X (71.6 AP)，尽管后者依赖于大规模的 Objects365 预训练。这些结果表明，当紧凑型 ViT 与任务专用蒸馏和边缘感知设计相结合时，可以成为边缘密集预测中实用且具有竞争力的选择。代码可在以下网址获取：https://intellindust-ai-lab.github.io/projects/EdgeCrafter/
