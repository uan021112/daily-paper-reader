...
tldr: 针对传统3D高斯泼溅在快速运动或恶劣光照下依赖高质量RGB和精确位姿的问题，本文提出E2EGS，一个仅基于事件流的无位姿3D重建框架。该方法利用事件流中的边缘信息进行轨迹估计和视图合成，通过时空相干性分析提取边缘，并引导高斯初始化与束调整。实验证明其在重建质量和轨迹精度上均优于现有方法，实现了全无位姿的事件驱动3D重建。
selection_source: fresh_fetch
motivation: 传统3D重建方法在高速运动或极端光照下因依赖RGB图像和已知位姿而表现不佳，而事件相机能提供高动态范围和高时间分辨率的结构信息。
method: 提出一种基于补丁的时间相干性分析方法来从噪声事件流中提取边缘，并利用这些边缘引导高斯初始化、位姿跟踪和束调整。
result: 在合成和真实数据集上的实验表明，E2EGS在重建质量和轨迹估计精度方面均达到了领先水平。
conclusion: E2EGS成功建立了一个完全无需预知位姿的事件驱动3D重建范式，显著提升了在复杂环境下的鲁棒性。
---
