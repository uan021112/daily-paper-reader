...
tldr: 针对Segment Anything Model (SAM) 在医疗影像分割中因提示框定位误差和固定阈值二值化导致的推理不稳定问题，本文提出了SAIF框架。该框架无需训练且即插即用，通过构建提示框扰动与阈值变化的联合不确定性空间，利用稳定性与边界一致性评分过滤并融合候选预测。实验证明，SAIF在多个医疗数据集上显著提升了分割精度与鲁棒性，无需架构修改即可达到SOTA性能。
selection_source: fresh_fetch
motivation: 解决SAM在医疗影像分割中因提示框误差和固定阈值导致的推理结果波动及边界不稳定性。
method: 通过结构化提示框扰动和阈值变化构建不确定性空间，并基于稳定性-一致性评分进行加权概率融合。
result: 在Synapse、CVC-ClinicDB等多个公开数据集上，SAIF在不改变模型权重的情况下显著提升了分割表现。
conclusion: SAIF证明了通过显式建模推理阶段的不确定性，可以有效增强预训练大模型在医疗领域的应用可靠性。
---
