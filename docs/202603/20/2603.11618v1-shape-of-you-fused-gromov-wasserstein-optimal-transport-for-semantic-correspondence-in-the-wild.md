...
title_zh: Shape-of-You：用于自然场景语义对应的融合 Gromov-Wasserstein 最优传输
authors: "Jiin Im, Sisung Liu, Je Hyeong Hong"
date: 2026-03-12
pdf: "https://arxiv.org/pdf/2603.11618v1"
tags: ["query:partseg"]
score: 7.0
evidence: 利用 3D 基础模型进行语义对应
tldr: 针对野外图像语义对应任务中 2D 基础模型缺乏结构一致性和几何歧义的问题，本文提出 Shape-of-You (SoY) 框架。该框架将伪标签生成建模为融合 Gromov-Wasserstein (FGW) 问题，结合 3D 基础模型提供的几何结构约束，并通过锚点线性化解决计算开销。SoY 在 SPair-71k 等数据集上达到 SOTA，实现了无需显式几何标注的高效语义匹配。
selection_source: fresh_fetch
motivation: 现有的无监督语义对应方法主要依赖 2D 特征的局部相似性，忽略了全局结构关系，导致在处理对称或重复特征时容易出现几何歧义。
method: 提出将伪标签生成重构为 FGW 问题，利用 3D 基础模型引入几何结构一致性，并通过锚点线性化近似和动态软目标损失实现高效鲁棒的学习。
result: 该方法在 SPair-71k 和 AP-10k 数据集上均取得了最先进的性能，显著提升了在复杂野外场景下的语义匹配精度。
conclusion: SoY 成功通过融合 3D 几何先验与 2D 特征优化，解决了语义对应中的结构一致性难题，为无监督视觉匹配提供了新思路。
---

## 摘要
语义对应对于处理缺乏显式对应标注的多样化自然场景图像至关重要。虽然最近的 2D 基础模型提供了强大的特征，但通过最近邻伪标签将其应用于无监督学习存在关键局限性：它仅在局部运行，忽略了结构关系，因此其对 2D 外观的依赖无法解决由对称性或重复特征引起的几何歧义。在这项工作中，我们通过将伪标签生成重新表述为融合 Gromov-Wasserstein (FGW) 问题来解决这一挑战，该问题共同优化了特征间相似性和结构内一致性。我们的框架 Shape-of-You (SoY) 利用 3D 基础模型在几何空间中定义这种内部结构，从而解决了上述歧义。然而，由于 FGW 是一个计算代价极高的二次规划问题，我们通过基于锚点的线性化对其进行近似。由此产生的概率传输计划提供了一个结构一致但带有噪声的监督信号。因此，我们引入了一种软目标损失，动态地将该计划的引导与网络预测相结合，以构建一个对这种噪声具有鲁棒性的学习框架。SoY 在 SPair-71k 和 AP-10k 数据集上达到了最先进的性能，在没有显式几何标注的情况下树立了语义对应的新基准。代码可在 Shape-of-You 获取。
