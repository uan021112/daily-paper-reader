...
tags: ["query:3dgen"]
score: 7.0
evidence: 从单张RGB图像进行3D手物姿态估计
tldr: 针对单张RGB图像中手-物姿态估计在遮挡和多样化交互下的挑战，本文提出GenHOI框架。该框架通过引入分层语义提示（涵盖物体状态、手部配置和交互模式）来增强模型的泛化能力，并结合多模态掩码建模策略和手部先验知识，在严重遮挡下实现可靠的姿态推理。实验证明其在DexYCB和HO3Dv2基准上达到了SOTA性能。
...
conclusion: 通过整合语义知识与几何先验，GenHOI有效提升了模型在复杂遮挡和未知交互场景下的泛化性与准确性。
---
