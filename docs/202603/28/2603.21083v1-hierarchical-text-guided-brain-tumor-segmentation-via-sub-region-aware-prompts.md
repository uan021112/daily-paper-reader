...
score: 8.0
evidence: 肿瘤子区域的分层分割
tldr: 针对脑肿瘤分割中子区域边界模糊及现有文本引导方法缺乏针对性的问题，本文提出TextCSP框架。该框架通过分层文本引导，利用子区域感知提示微调和软级联解码器，实现了从整体肿瘤到增强肿瘤的由粗到精分割。通过引入文本语义通道调制器，模型能更精准地捕捉临床描述特征，在TextBraTS数据集上显著提升了Dice和HD95指标。
selection_source: fresh_fetch
motivation: 现有的多模态脑肿瘤分割方法通常使用单一全局文本嵌入，忽略了不同肿瘤子区域独特的临床特征和解剖包含关系。
method: 提出TextCSP框架，包含由粗到精的软级联解码器、基于LoRA和软提示的子区域感知文本编码器，以及用于特征增强的文本语义通道调制器。
result: "在TextBraTS数据集上，该方法在Dice和HD95指标上分别比现有最先进方法提升了1.7%和6%。"
conclusion: 结合子区域特异性文本提示的分层引导策略能有效提升脑肿瘤分割的准确性，为多模态医学影像分析提供了新思路。
---
