...
tldr: PCSTracker是首个专门为点云序列设计的一端到端长程场景流估计框架。针对现有方法在处理长序列时存在的几何演变、遮挡和误差累积等问题，该方法引入了迭代几何运动联合优化模块和时空轨迹更新模块，并结合重叠滑动窗口推理策略。实验证明，该方法在保持32.5 FPS实时性能的同时，在多个数据集上实现了领先的长程运动估计精度。
selection_source: fresh_fetch
motivation: 现有的点云场景流估计方法多局限于两帧设置，难以在长序列中应对几何变化、遮挡及误差累积导致的运动不一致性。
method: 提出了包含迭代几何运动联合优化、时空轨迹更新以及重叠滑动窗口推理策略的端到端框架。
result: 在PointOdyssey3D和ADT3D数据集上达到了最优的长程场景流估计精度，并实现了32.5 FPS的实时运行速度。
conclusion: PCSTracker有效解决了点云长程运动分析中的一致性难题，为3D场景理解提供了高效且准确的解决方案。
---
